...
plot2 = ggplot(combined_data, aes(x = year, y = mean, color = scenario)) +
  geom_line(linewidth = 0.8) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = scenario), alpha = 0.12, color = NA, na.rm = TRUE) +
  scale_x_continuous(limits = c(2009, NA)) +  # Set x-axis to start at 2009
...
grid.arrange(
  arrangeGrob(plot1, plot2, plot3, plot4, nrow = 2, ncol = 2),  # Grid of plots
