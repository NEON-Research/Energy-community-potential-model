...
create_installed_capacity_plot <- function(){  
  ...
  historical_data <- read_excel(file_path_historic, sheet = "calibration_statistics") %>%
    select(year, `Members (% of households)`) %>%
    rename(median = `Members (% of households)`) %>%
  ...
  # Convert projects to members (94 % households per project)
  convert_to_mb <- function(df) df[, -1] * 93 / 8400000 * 100
  #convert_to_gw_historical <- function(df) df[, -1] / 1000000
  #historical_data_gw <- convert_to_gw_historical(historical_data)
  data_scenario1_mb <- convert_to_mb(data_scenario1_filtered)
  data_scenario2_mb <- convert_to_mb(data_scenario2_filtered)
  data_scenario3_mb <- convert_to_mb(data_scenario3_filtered)
  data_scenario4_mb <- convert_to_mb(data_scenario4_filtered)
  ...
  # Compute statistics
  scenario1_stats <- get_mean_and_ci(data_scenario1_mb)
  scenario2_stats <- get_mean_and_ci(data_scenario2_mb)
  scenario3_stats <- get_mean_and_ci(data_scenario3_mb)
  scenario4_stats <- get_mean_and_ci(data_scenario4_mb)
  ...
  plot <- ggplot(combined_data, aes(x = year, y = mean, color = scenario)) +
    ...
    labs(title = "Members (% of households)",
         x = "Year",
         y = NULL,
         color = "Scenario",
         fill = "Scenario") +
...
ggsave("plot_scenarios_membership.png", plotInstalledCapacity, width = 8, height = 4.5, dpi = 300)
